...
import { IGetTvResult } from "../Routes/Tv";
import { getTv, getTvCredit } from "../api";
...
const TvSlider = ({ category }: { category: TvCurrent }) => {
  const { data } = useQuery<IGetTvResult>(["tv", category], () =>
    getTv(category)
  );
  ...
  const bigMovieMatch = useMatch(`/React_netflix/tv/${category}/:movieId`);
  ...
  return (
    <>
      <H3 onClick={incraseIndex}>{category}</H3>
      <Sliders>
        <ArrowBtn
            ...
            >
              {clickedMovie && (
                <>
                  ...
                  <BigOverview>
                    <b>{clickedMovie.first_air_date.slice(0, 4)}</b> |{" "}
